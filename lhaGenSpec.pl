...
/*A1A ***Genrates the probe clauses***COMMENTED by GNB on Nov10 2009 %BEGINS
genSpec(Tree,S1) :-
...
*A1A***COMMENT ENDS****Nov10 2009 %ENDS */
...
genSpec(Tree,S1) :-
	...
	ratePart(Locs,VarTable,S6,S7),
	gammaPart(Transitions,0,VarTable,S7,S8),
	%alphaPart(Transitions,0,VarTable,S8,S9),
	%probePart(VarTable,0,S9,[]).
	alphaPart(Transitions,0,VarTable,S8,[]).
...
standardPart(VarTable,[locationOf(['$VAR'('Loc')|UList],'$VAR'('Loc'))|S0],S0) :-
...
variablePart(VarTable,[C1,C2|S0],S0) :-
	...
	C1 = (before([U|StateNow],[U|StateLater]) :- T =< T1),
	generateList(U,N,UnList),
	C2 = (stateSpace([L|UnList]) :- location(L)).


locationPart([],S0,S0).
locationPart([locdecl(LocId,_,_)|LocDecls],[location('$VAR'(LocId))|S0],S1) :-
...
initPart([init(Loc,Alpha)|Inits],VarTable,[C|S0],S1) :-
...
assignments([action(ident(V):=ident(X))|Actions],VarTable,[J|AssignedVarsIndexs],[X|AssignedVals]) :-
	write('assignments called'),nl,
	member(var(V,'$VAR'(J),_),VarTable),
	write('member call succeeded'),nl,
	write('Actions evaluates to:'),nl,
...
invariantPart([],_,S0,S0).
invariantPart([LocDecl|LocDecls],VarTable,[(H :- B),(NegH :- NegB)|S0],S1) :-
	invariantHead(LocDecl,VarTable,H),
...
ratePart([],_,S0,S0).
ratePart([LocDecl|Locs],VarTable,[(H :- B)|S0],S1):-
...
gammaPart([],_,_,S0,S0).
gammaPart([Transition|Transitions],K,VarTable,[(H :- B)|S0],S1) :-
...
alphaPart([],_,_,S0,S0).
alphaPart([Transition|Transitions],K,VarTable,[(H :- B)|S0],S1):-
...
probePart(VarTable,IndexForDollarVar,[(H :- B)|S1],S0) :-
	length(VarTable,N),
